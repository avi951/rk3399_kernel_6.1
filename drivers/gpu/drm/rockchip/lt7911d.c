...
static int lt7911d_connector_get_modes(struct drm_connector *connector)
{
	...
	/* if (lt7911d->edid)
		return drm_add_edid_modes(connector, lt7911d->edid); */

	lt7911d->edid = (struct edid *)edid;
	drm_mode_connector_update_edid_property(connector, lt7911d->edid);

	num_modes = drm_add_edid_modes(connector, lt7911d->edid);
...
// static int lt7911d_probe(struct device *dev)
{
	...
	lt7911d->reset_n = devm_gpiod_get(dev, "reset", GPIOD_OUT_HIGH);
...
static struct platform_driver lt7911d_bridge_driver = {
    ...
    .driver = {
        .name = "lt7911d",
        .of_match_table = of_match_ptr(lt7911d_bridge_match),
    },
};

/* static int __init lt7911d_driver_init(void)
{
	return platform_add_driver(&lt7911d_bridge_driver);
}

static int __exit lt7911d_driver_exit(void)
{
	platform_del_driver(&lt7911d_bridge_driver);
}
