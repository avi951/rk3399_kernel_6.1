...
static void lt7911d_format_change(struct v4l2_subdev *sd)
{
	...
	{
		if (!v4l2_match_dv_timings(&lt7911d->timings, &timings, 0))
		{
			enable_stream(sd, false);
						   ...
						   struct v4l2_subdev_format *format)
{
	...
	format->format.field =
		lt7911d->timings.bt.interlaced ? V4L2_FIELD_INTERLACED : V4L2_FIELD_NONE;
...
static int lt7911d_probe_of(struct lt7911d_state *lt7911d)
{
	...
	lt7911d->reset_gpio = devm_gpiod_get_optional(dev, "reset", GPIOD_OUT_HIGH);
						 ...
						 const struct i2c_device_id *id)
{
	...
	err = devm_request_threaded_irq(dev, lt7911d->plugin_irq, NULL,
									plugin_detect_irq_handler, IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_ONESHOT, "lt7911d",
									lt7911d);
	if (err)
